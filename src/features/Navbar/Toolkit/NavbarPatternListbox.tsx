...
import { MajorChord } from "presets/patterns/1-BasicChords";
...
export function NavbarPatternListbox() {
  ...
  const addingPatternClips = isTimelineAddingPatternClips(timeline);

  // Set the selected pattern to the first pattern in the list.
  useEffect(() => {
    if (!selectedPattern) {
      dispatch(setSelectedPattern(patterns[0]?.id || MajorChord.id));
    }
  }, [selectedPattern, patterns]);

  // Compile the pattern groups with the custom patterns
  const PatternGroups: PatternGroupMap = {
    ...PresetPatternGroupMap,
  ...
  const isPatternInCategory = (pattern: Pattern, category: PatternGroupKey) => {
  ...
  const PatternListboxButton = ({ open }: { open: boolean }) => (
    <Listbox.Button
  ...
  const PatternDropdownMenu = ({ open }: { open: boolean }) => (
    <Transition
      show={open}
      appear
      ...
      >
        {({ open }) => (
          <div className="relative">
            <PatternListboxButton open={open} />
  ...
  const PatternLabel = () => (
    <span
      className={classNames(
        ...
        { "text-teal-500": addingPatternClips },
        { "text-emerald-500": !addingPatternClips }
      )}
    >
      Pattern
    </span>
  ...
  const className = classNames(
    ...
    { "border-green-400 ring-1 ring-green-400": onPatternEditor },
    { "border-teal-500 ring-1 ring-teal-500": addingPatternClips },
    { "border-slate-400/80": !onPatternEditor && !addingPatternClips }
  );
