...
import { deleteSample, downloadSample, SampleData } from "app/samples";
  ...
  return (
    <HomeListItem key={key} className="border-2 border-cyan-500/70">
      <GiSoundWaves
        ...
        className="max-[800px]:hidden cursor-pointer size-48 mt-4 mb-4 ease-out p-3 rounded-full border-2 transition-all active:text-indigo-200 hover:duration-150 border-teal-400/50 ring-teal-600/25 ring-offset-8 ring-offset-teal-500/25 bg-radial from-indigo-700 to-teal-500 data-[deleting=true]:from-red-500 data-[deleting=true]:to-red-700 shadow-[0px_0px_20px_rgb(100,100,200)]"
      />
      ...
      <div className="flex w-full grow px-2 mb-auto text-slate-50 group">
        <div className="w-full my-auto bg-slate-900/50 p-3 border-2 rounded border-indigo-400/50 select-none text-lg font-thin">
          ...
          <HomeListSubtitle
            ...
            body={dayjs(uploadDate).format("MM/DD/YY [@] h:mm:ss a")}
          />
          <HomeListSubtitle
            title="Samplers:"
        ...
        {/* Collapsed icon */}
        <GiAudioCassette className="min-[800px]:hidden border-2 p-2 text-indigo-200/80 my-auto text-7xl rounded-full border-indigo-400/50 ring-indigo-600/25 ring-offset-8 ring-offset-indigo-500/25 bg-radial from-indigo-700 to-sky-500 shadow-[0px_0px_20px_rgb(100,100,200)]" />
      </div>
      <HomeListButtonContainer>
        ...
        <HomeListButton onClick={() => downloadSample(key)}>
          Save
        </HomeListButton>
        <HomeListButton
          border={`border ${
          ...
          onClick={() => (!deleting ? setDeleting(true) : deleteSample(key))}
