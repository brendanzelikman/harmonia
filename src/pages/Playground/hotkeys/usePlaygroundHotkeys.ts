...
import { CLOSE_STATE, getToggledState, TOGGLE_STATE } from "hooks/useToggle";
...
export const UNDO_PROJECT_HOTKEY: Thunk<Hotkey> = (dispatch, getProject) => ({
  ...
  shortcut: "meta+z",
  callback: () =>
    selectCanUndo(getProject()) && dispatch({ type: UNDO_PROJECT_TYPE }),
});
...
export const REDO_PROJECT_HOTKEY: Thunk<Hotkey> = (dispatch, getProject) => ({
  ...
  callback: () =>
    selectCanRedo(getProject()) && dispatch({ type: REDO_PROJECT_TYPE }),
});
...
export const RECORD_TRANSPORT_HOTKEY: Thunk<Hotkey> = (dispatch) => ({
  ...
  shortcut: "shift+r",
  callback: () => {
    !!getToggledState(RECORD_TRANSPORT)
      ? stopRecordingTransport()
      : startRecordingTransport();
  },
});
