...
export function LandingPage() {
  ...
  const HeroImage = (props: { src: string }) => (
    <img
      src={props.src}
      className="w-1/2 border-2 border-indigo-400/25 shadow-xl shadow-sky-600/50 rounded-xl"
    />
  );
  ...
  const PatternHero = () => (
    <Section>
      ...
      <div className="w-full flex flex-row flex-wrap justify-center my-4 mt-8">
        ...
        <HeroQuote
          title="Compose and Transpose Your Ideas"
  ...
  const LibraryHero = () => (
    <Section>
      <div className="w-full items-center justify-center flex flex-row flex-wrap px-4 space-x-6 [&>a>img]:shadow-2xl [&>a]:md:w-1/6 [&>a]:w-[100px] [&>a>img]:max-w-[200px] [&>a>img]:w-full">
      ...
      <div className="text-center mt-16 font-bold lg:text-6xl md:text-4xl text-2xl text-slate-100 shadow-2xl">
  ...
  const ContactHero = () => (
    <Section>
      ...
      <div className="mt-16 text-center font-bold md:text-6xl text-4xl text-slate-100 shadow-xl">
...
export const MainButton = (props: { onClick?: () => void }) => {
  return (
    <button
      ...
      className="border py-5 px-8 text-slate-100 bg-slate-800/20 hover:bg-sky-950/90 hover:shadow-[0px_0px_10px_5px_rgb(15,150,200)] transition-all duration-150 border-slate-500 rounded-xl backdrop-blur shadow-xl sm:text-4xl text-sm font-light"
    >
      Make Music Now!
    </button>
  );
